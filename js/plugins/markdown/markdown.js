...
(function (root, factory) {
  ...
  var DEFAULT_NOTES_SEPARATOR = 'notes?:'
    ...
    for (var i = 0, len = sectionStack.length; i < len; i++) {
      ...
      if (sectionStack[i] instanceof Array) {
        ...
        sectionStack[i].forEach(function (child) {
          markdownSections += '<span data-markdown>' + createMarkdownSlide(child, options) + '</span>'
        })

        markdownSections += '</span>'
      } else {
        markdownSections += '<span ' + options.attributes + ' data-markdown>' + createMarkdownSlide(sectionStack[i], options) + '</span>'
      }
    }
